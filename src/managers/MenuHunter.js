...
export class MenuHunter {
  ...
  create() {
    ...
    this.#skinIndex = Math.max(0, SKINS.findIndex(sk => sk.id === profile.activeSkin));
  ...
  draw(ctx) {
    if (!this.swing || !this.#visible) return;
    const sw = this.swing;
    ...
    // Крюк
    this.#drawHook(ctx, sw.anchorX, sw.anchorY);
    ...
    drawSkinPose(ctx, this.#skinIndex, 0);
    ctx.restore();
